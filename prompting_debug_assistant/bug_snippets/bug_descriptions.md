...
**Notes**: The loop variable `i` is declared with `var`, making it function-scoped (or global if not in a function). The `setTimeout` callbacks are executed after the main loop has finished. Since `var` does not have block scope, all callbacks share the same reference to the variable `i`. By the time the timers fire, the loop has completed and `i` has a value of 5, so all callbacks print "index 5" instead of the captured index.
